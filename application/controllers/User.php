...
class User extends CI_Controller {
    ...
    public function update() {
        ...
        $this->Md->update($id, $student, 'user');
        ...
        if (!empty($_POST)) {
            ...
            foreach ($_POST as $field_name => $val) {
                ...
                if (!empty($user_id) && !empty($field_name) && !empty($val)) {
                    ...
                    $this->Md->update($user_id, $student, 'user');
                    echo "Updated";
                } else {
    ...
    public function reset() {
        ...
        $email = $this->Md->query_cell("SELECT email FROM user WHERE id ='" . $id . "'", 'email');
        $name = $this->Md->query_cell("SELECT fname FROM user WHERE id ='" . $id . "'", 'fname');

        $new_password = $this->encrypt->encode($password, $email);

        $info = array('password' => $new_password);
        $this->Md->update_dynamic($id, 'id', 'user', $info);
    ...
    public function update_image() {
        ...
        if (!$this->upload->do_upload($file_element_name)) {
            ...
            redirect('/management/details/' . $userID, 'refresh');
